...
void OCR_MainWindow::configImage()
{
    ...
    registerOCRSignal();
    QString savingPath =  PDF_ImageSaver::imageSaverNameProducer(
                            this->imageSavePath_DIR,
                            PDF_LocationDefaults::getFileNameByPath(this->pdfPath)).remove(".pdf");

    PDF_ImageSaver::saveImage(ui->image_label->grab().toImage(), savingPath);
...
{
    if( this->imageSavePath_DIR.isEmpty() || // path empty
        !PDF_LocationDefaults::isFileExsits(this->imageSavePath_DIR) ||
        !PDF_LocationDefaults::isADir(this->imageSavePath_DIR)
        ){ // invaild path
        return false;
    }

    QDir dir(this->imageSavePath_DIR);

    bool st =  dir.removeRecursively();
    dir.mkdir(this->imageSavePath_DIR);
    return st;
}

bool OCR_MainWindow::clearRes()
{
    if( this->resultSavePath_DIR.isEmpty() || // path empty
        !PDF_LocationDefaults::isFileExsits(this->resultSavePath_DIR) ||
        !PDF_LocationDefaults::isADir(this->resultSavePath_DIR)
        ){ // invaild path
        return false;
    }

    QDir dir(this->resultSavePath_DIR);
    bool st =  dir.removeRecursively();
    dir.mkdir(this->resultSavePath_DIR);
    return st;
}
