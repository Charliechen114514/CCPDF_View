...
void MainWindow::initExportConnection()
{
    if(!this->exporterAndImageHolder)
        return;
    connect(this->exporterAndImageHolder, &PDF_ExporterAndImageHolder::finishRenderIndex,
            this, &MainWindow::showRenderProcess);

    connect(this->exporterAndImageHolder, &PDF_ExporterAndImageHolder::finishRenderAll,
            this, &MainWindow::showRenderFinish);

    connect(this->exporterAndImageHolder, &PDF_ExporterAndImageHolder::finishExportPg,
            this, &MainWindow::showExportProcess);

    connect(this->exporterAndImageHolder, &PDF_ExporterAndImageHolder::finishExport,
            this, &MainWindow::showExportFinish);

    connect(this->exporterAndImageHolder, &PDF_ExporterAndImageHolder::startRender,
...
void MainWindow::initExportReminder()
{
    ...
    reminderWindow = new ReminderOfExportsMainWindow(this);
    reminderWindow->setPageTotal(ui->pdf_view->currentPageCnt());
    connect(this, &MainWindow::updatePageValue,
            reminderWindow, &ReminderOfExportsMainWindow::updateProcessing);

    connect(this, &MainWindow::updateToExportTag,
            reminderWindow, &ReminderOfExportsMainWindow::switchToExport);

    connect(this, &MainWindow::updateToTellFinishExport,
            reminderWindow, &ReminderOfExportsMainWindow::finishAndShallClose);

    connect(this->reminderWindow, &ReminderOfExportsMainWindow::cancelTheSave,
            this, &MainWindow::cancelTheSave);


    if(this->exporterAndImageHolder)
        connect(this->exporterAndImageHolder, &PDF_ExporterAndImageHolder::cancelFinish
                ,this->reminderWindow, &ReminderOfExportsMainWindow::handleTheCancel);
}
...
bool MainWindow::saveForOtherCopy()
{
    ...
    if(exporterAndImageHolder && exporterAndImageHolder->isExportingCurrent())
    {
    ...
    if(!exporterAndImageHolder)
        exporterAndImageHolder = new PDF_ExporterAndImageHolder(ui->pdf_view->getDocForExport(), this);
    else
        exporterAndImageHolder->restartForRenderWork(ui->pdf_view->getDocForExport());
    initExportConnection();
    initExportReminder();
    exporterAndImageHolder->registerTheEdited(&editedImages);
    exporterAndImageHolder->setExportPath(savePath);
    exporterAndImageHolder->doAllWorkOneByOne();
    return true;
}
...
void MainWindow::cancelTheSave()
{
    ...
    disconnect(this, &MainWindow::updatePageValue,
            reminderWindow, &ReminderOfExportsMainWindow::updateProcessing);

    disconnect(this, &MainWindow::updateToExportTag,
            reminderWindow, &ReminderOfExportsMainWindow::switchToExport);
...
{
    emit updatePageValue(pg);
}

void MainWindow::showRenderFinish()
{
    emit updateToExportTag();
}
