...
class CCPDFView_MainWindow : public QMainWindow {
    Q_OBJECT
public:
    enum class PageNavigateCommand { PAGE_NEXT, PAGE_PREV, PAGE_BEG, PAGE_END };

    enum class PageModeBrowseCommand { SINGLE_PAGE, MULTI_PAGE };

    enum class PageZoomCommand { ZOOM_IN, ZOOM_OUT };
    enum PAGE_INDEX {
        HISTMENU,
        PAGE_NAV,
        OCR_TEXT,
    ...
    // ------------------------------------------------------
    //  Exports neccessary WidgetHandler
    // ------------------------------------------------------
    QTextBrowser* textBrowser() const;
    ~CCPDFView_MainWindow();

public slots:
    __BIND_Ui_Widgets void pageForward() {
        pageNavigate(PageNavigateCommand::PAGE_NEXT);
    }
    __BIND_Ui_Widgets void pageBackward() {
        pageNavigate(PageNavigateCommand::PAGE_PREV);
    }
    __BIND_Ui_Widgets void pageBegin() {
        pageNavigate(PageNavigateCommand::PAGE_BEG);
    }
    __BIND_Ui_Widgets void pageEnd() {
        pageNavigate(PageNavigateCommand::PAGE_END);
    }
    __BIND_Ui_Widgets void zoomIn() {
        zoom(PageZoomCommand::ZOOM_IN);
    }
    __BIND_Ui_Widgets void zoomOut() {
        zoom(PageZoomCommand::ZOOM_OUT);
    }
    __BIND_Ui_Widgets void pageSingle() {
        setPageMode(PageModeBrowseCommand::SINGLE_PAGE);
    }
    __BIND_Ui_Widgets void pageMulti() {
        setPageMode(PageModeBrowseCommand::MULTI_PAGE);
    }
    __BIND_Ui_Widgets void routeToHistory() {
        routeTo(PAGE_INDEX::HISTMENU);
    }
    __BIND_Ui_Widgets void routeToSearching() {
        routeTo(PAGE_INDEX::SEARCHIN);
    }
    __BIND_Ui_Widgets void routeToLibrary() {
        routeTo(PAGE_INDEX::LIBRARY);
    }
    __BIND_Ui_Widgets void routeToTranslate() {
        routeTo(PAGE_INDEX::TRANSLATE);
    }
    __BIND_Ui_Widgets void routeToLink() {
        routeTo(PAGE_INDEX::LINK);
    };
    __BIND_Ui_Widgets void routeToOCR() {
        routeTo(PAGE_INDEX::OCR_TEXT);
    }
...
private slots:
    __BIND_Ui_Widgets void on_btn_pageBack_clicked() {
        pageBackward();
    }
    __BIND_Ui_Widgets void on_btn_pageForward_clicked() {
        pageForward();
    }
    __BIND_Ui_Widgets void on_btn_copyText2ClipBoard_clicked() {
        copyPdfTextToClipBoard();
    }
    __BIND_Ui_Widgets void on_btn_toBegin_clicked() {
        pageBegin();
    }
    __BIND_Ui_Widgets void on_btn_toEnd_clicked() {
        pageEnd();
    }
    __BIND_Ui_Widgets void on_btn_sigPage_clicked() {
        pageSingle();
    }
    __BIND_Ui_Widgets void on_btn_multiPage_clicked() {
        pageMulti();
    }
    __BIND_Ui_No_Inline void
    on_historical_tableWidget_customContextMenuRequested(const QPoint& pos);
    __BIND_Ui_No_Inline void on_historical_tableWidget_itemClicked(
...
private:
    ...
    bool init_loadRecordPDF(const PDF_Info_Historical_Record& rec,
                            bool                              if_isLastOne);
    ...
    //  handle Search Jump
    // ------------------------------------------------------
    void handleSearchJump(const int page, QPointF location,
    ...
    // ------------------------------------------------------
    //  SpeechWidget Related
    // ------------------------------------------------------
    void slot_doNextPageAndSaying();
#endif
    // ------------------------------------------------------
    // Private Members
    // ------------------------------------------------------
    ...
    __MEMEROY_HOLD_BY_SMT_PTR
    std::unique_ptr<CCPDF_Other_Simple_Server_HistoricalHelper>
        otherServerRecorder;
    __MEMEROY_HOLD_BY_SMT_PTR
    ...
    __MEMEROY_HOLD_BY_OBJ_TREE
    std::unique_ptr<CCPDF_HistoryTableWidgetController>
                                             histtableWidgetController;
    std::unique_ptr<CCPDF_ThemeActionHelper> themeActionHelper;
    __MEMEROY_HOLD_BY_OBJ_TREE
    CCPDF_HelpWindow* helpWindow{nullptr};
#ifdef SUPPORT_SPEECHING
    __MEMEROY_HOLD_BY_OBJ_TREE
    SpeechWidgets* _speechWidget{};
#endif
};
